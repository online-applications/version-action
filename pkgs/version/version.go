...
func GetLatestTag(environment string) (string, error){
	...
	out_test, err_test := exec.Command("sh", "-c", "git tag -l").Output()
	if err_test != nil {
		log.Println("Error was found while getting the latest commit message", err_test)
	}
	log.Println("Fetched tag:", string(out))
	log.Println("Fetched out_test:", string(out_test))
...
func GetVersionType(input string, words [3]string) VersionType {
	versionType := VersionType{}
	versionType.Contains = true
	...
	switch true {
	case v1:
		versionType.VersionType =  words[0]
		return versionType
	case v2:
		versionType.VersionType =  words[1]
		return versionType
	case v3:
		versionType.VersionType =  words[2]
		return versionType
	}
	versionType.Contains = false
